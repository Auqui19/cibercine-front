...
const Confiteria = () => {
  ...
  const addComboToOrder = (combo) => {
    ...
    setTotalPrice((prevTotal) => prevTotal + combo.price);
  };

  const handleContinue = () => {
    const comboDetails = selectedCombos.map(combo => `${combo.name} S/ ${combo.price}`).join(', ');
    const queryString = `?movie=${movieTitle}&seats=${selectedSeats}&date=${date}&time=${time}&combos=${comboDetails}&totalPrice=${totalPrice.toFixed(2)}`;
